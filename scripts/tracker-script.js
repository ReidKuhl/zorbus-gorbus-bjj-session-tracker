// Read username from URL and set it
...
window.openModal = function (type) {
  ...
  if (type === 'practice') {
    ...
    ['p-mins', 'p-notes', 'p-techs', 'p-school'].forEach(id => document.getElementById(id).value = '');
  } else {
    document.getElementById('c-date').value = today;
    ['c-name', 'c-division', 'c-wins', 'c-losses', 'c-notes'].forEach(id => document.getElementById(id).value = '');
  }
...
// Edit entry — populates the existing modal and switches save to update
window.editEntry = function (type, id) {
  if (type === 'practice') {
    ...
    const saveBtn = document.querySelector('#modal-practice .btn:not(.btn-ghost)');
    saveBtn.textContent = 'Update Session';
    saveBtn.onclick = () => updateEntry('practice', id);
    ...
    const saveBtn = document.querySelector('#modal-comp .btn:not(.btn-ghost)');
    saveBtn.textContent = 'Update Competition';
    saveBtn.onclick = () => updateEntry('comp', id);
    document.getElementById('modal-comp').classList.add('open');
  }
};

// Reset modal save button back to default after close
const origClosePractice = window.closeModal;
window.closeModal = function (type) {
  document.getElementById('modal-' + type).classList.remove('open');
  if (type === 'practice') {
    const saveBtn = document.querySelector('#modal-practice .btn:not(.btn-ghost)');
    saveBtn.textContent = 'Save Session';
    saveBtn.onclick = () => saveEntry('practice');
  } else {
    const saveBtn = document.querySelector('#modal-comp .btn:not(.btn-ghost)');
    saveBtn.textContent = 'Save Competition';
    saveBtn.onclick = () => saveEntry('comp');
  }
};
...
document.getElementById('belt-select').addEventListener('change', window.saveBelt);
document.getElementById('stripe-select').addEventListener('change', window.saveBelt);
...
function renderPractices() {
  ...
  if (!practices.length) {
    list.innerHTML = '<div class="empty-state"><div class="empty-glyph">OSS</div><div class="empty-msg">No sessions logged yet. Step onto the mat.</div></div>';
    return;
  }
  list.innerHTML = '';
  practices.forEach(e => {
    ...
    card.innerHTML = `
      ...
      <div class="entry-body">
        ...
        <div class="entry-meta">${meta.join(' &middot; ')}</div>
      ...
      <div class="entry-actions">
        <button class="icon-btn" onclick="editEntry('practice','${e.id}')">&#x270E;</button>
        <button class="icon-btn del" onclick="deleteEntry('practice','${e.id}')">&#x2715;</button>
      </div>`;
...
function renderComps() {
  ...
  document.getElementById('comp-record').innerHTML = `<span class="win">${tw}</span><span class="sep"> &ndash; </span><span class="loss">${tl}</span>`;
  if (!comps.length) {
    list.innerHTML = '<div class="empty-state"><div class="empty-glyph">&#x1F94B;</div><div class="empty-msg">No competitions logged yet.</div></div>';
    return;
  }
  list.innerHTML = '';
  ...
  comps.forEach(e => {
    ...
    card.innerHTML = `
      ...
      <div class="entry-body">
        <div class="entry-row1"><span class="entry-type-tag comp">comp</span><span class="entry-title">${e.name}</span></div>
        <div class="entry-meta">${e.division ? e.division + ' &middot; ' : ''}${e.wins}W ${e.losses}L</div>
        <div style="margin-bottom:0.4rem;"><span class="result-badge ${e.result}">${rl[e.result] || e.result}</span></div>
        ${e.notes ? `<div class="entry-notes">${e.notes}</div>` : ''}
      </div>
      <div class="entry-actions">
        <button class="icon-btn" onclick="editEntry('comp','${e.id}')">&#x270E;</button>
        <button class="icon-btn del" onclick="deleteEntry('comp','${e.id}')">&#x2715;</button>
      </div>`;
    list.appendChild(card);
  });
}
...
function renderStats() {
  ...
  const mins = practices.reduce((a, e) => a + e.mins, 0);
  const now = new Date();
  ...
  document.getElementById('s-streak2').textContent = calcStreak();
...
function updateHeaderStats() {
  document.getElementById('stat-total').textContent = practices.length;
...
function renderBelt() {
  ...
  document.getElementById('belt-current-label').innerHTML =
    `${beltNames[beltData.belt]} Belt${beltData.stripes > 0 ? ' · ' + beltData.stripes + ' Stripe' + (beltData.stripes > 1 ? 's' : '') : ''}`;
  document.getElementById('belt-select').value = beltData.belt;
  document.getElementById('stripe-select').value = beltData.stripes;
}

function renderAll() { renderPractices(); renderComps(); updateHeaderStats(); renderBelt(); }
